            ...
            opt_theme: {
                ...
                handler: function(token) {
                    if (token) {
                        ...
                        $("body").css("cssText", "background-color: #000!important");
                    }
                }
            },
            opt_self_name: {
...
function ChatRoom() {
    ...
    this.sendMessageToServer = message => {
        ...
        if (message.length < 200 && message.length > 0) {
            const view = new DataView(new ArrayBuffer(2 + 2 * message.length));
            let offset = 0;
    ...
    this.createChatBox = function() {
        ...
        $("#overlays2").append('<div id="respawning" style="display: none; width: 100%; height: 100%; margin-top: -1%;"><h2 class="respawning" style="text-align: center;width: 22%;height: 6%;margin-top: 100px;background: rgba(0, 0, 0, 0.64);padding-top: 12px;margin-left: 40%;border-radius: 15px;">Respawning in 3 seconds...</h2><div id="loading" style=" height: .3%; width: 0%; margin-left: 40%; margin-top: -9px; transition-property: all; transition-duration: 3s;    background: #27ff00; "></div></div>');
        this.hide();
        $("#chatboxArea2").hide();
    ...
    this.receiveMessage = function(msg, message) {
        ...
        if ("\u4e0d\u7559\u540d [slick]" == msg) {
            size.html(this.replaceIcon(size.html()));
        }
        tabContent.append(errors);
        tabContent.append(size);
        errors = $("<span class='msg'>").text(message);
        errors.html(this.replaceIcon(errors.html()));
        tabContent.append(errors);
        $("#chatroom").append(tabContent);
        this.scrollDown();
    };
    this.replaceIcon = function(xhtml) {
        var query;
        for (query in Icon) {
            xhtml = xhtml.replace(new RegExp(escapeRegex(query), "g"), '<img alt="$1" src="' + Icon[query] + '">');
        }
        var images = $('img[alt="$1"]');
        var i = 0;
        for (; i < images.length; i++) {
            for (query in Icon) {
                if (images[i].src == Icon[query]) {
    ...
    function parse(view) {
        ...
        switch (240 == view.getUint8(offset) && (offset += 5), view.getUint8(offset++)) {
            ...
            case 99:
                ...
                let str = '';
                let color = '#000000';
                const dataView = view.getUint8(offset++);
                if (dataView & 2) {
                    offset += 4;
                }
                if (dataView & 4) {
                ...
                if (extra) {
                    extraBytes = readFile();
                    try {
                        extras = JSON.parse(extraBytes);
                    } catch (e) {}
                }
                chatRoom.receiveMessage(name, message, color);
                break;
        }
    }
...
}, drawMinimapNodes(), DisableZoom(), clearOldNodesData(), updateLbDiv(), updateScoreDiv(), $.cachedScript("https://cdnjs.cloudflare.com/ajax/libs/jquery.perfect-scrollbar/0.7.0/js/perfect-scrollbar.jquery.min.js").done(function(dataAndEvents, deepDataAndEvents) {
...
/*setTimeout(function(){
    setInterval(function() {
        ws = new WebSocket('ws://49.12.231.126:7251');
    window.webSocket = ws;
    window.urlSocket = "ws://49.12.231.126:7251";
    var d = Date.now()
    ws.binaryType = "arraybuffer";
    ws.onopen = function() {
    document.getElementById('pingInput').value=Math.floor(Date.now()-d);
    };},2000)
},10000)*/
...
document.getElementById("chatroom").innerHTML+=`<div><span class="time">${time} </span><span class="sender">SERVER : </span><span class="msg"><a class="iDSClink" onclick="window.open('http://discord.gg/DRASUGDRQp')">discord.gg/DRASUGDRQp</a></span></div>`
